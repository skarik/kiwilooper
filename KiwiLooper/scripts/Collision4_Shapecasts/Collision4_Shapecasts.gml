...
function collision4_rectanglecast2(rectOrigin, rectSizeX, rectSizeY, rectDir, rectDist, hitMask)
{
	...
	// Okay, so now we're going to try to do a 'fast frustum test' on the bbox
	...
	if (collision4_rectflat3_test(
		rectOrigin, rectDir, 1024,
		Plane3FromNormalOffset(new Vector3(1, 0, 0), rectOrigin), rectSizeX * 0.5,
		Plane3FromNormalOffset(new Vector3(0, 1, 0), rectOrigin), rectSizeY * 0.5,
		unused_objects, hit_distances, hit_normals, hitMask, false, []))
	{
		if (!bHasHit || (hit_distances[0] < hitDistance))
		{
			bHasHit = true;
